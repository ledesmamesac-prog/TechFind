...
import { catIcon, getStoreBadgeClass, renderStarsHtml } from '../dashboard/icons.js';
...
const favoritesHeader = document.querySelector('.favorites-page-header');
const favoritesToolbar = document.querySelector('.favorites-page-toolbar');
const profileOverlay = document.getElementById('profile-modal-overlay');
const logoutBtn = document.getElementById('logout-btn');

let currentUser = null;
let favorites = null; // null means loading
...
function renderFavorites() {
  if (favorites === null) return; // Still loading data
  ...
  // Toggle sections
  if (favoritesEmpty) favoritesEmpty.hidden = hasFavorites;
  if (favoritesGrid) favoritesGrid.hidden = !hasFavorites;
  if (favoritesToolbar) favoritesToolbar.hidden = !hasFavorites;

  // Toggle specific header elements instead of the whole header
  if (clearFavoritesBtn) clearFavoritesBtn.hidden = !hasFavorites;
  if (favoritesCount) favoritesCount.hidden = !hasFavorites;
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (!user) {
    favorites = []; // No user means no favorites (empty)
    renderFavorites();
    return;
  }
